...
public  class SatelliteTabulator {
    ...
    public SatelliteTabulator(Context context) {
     ...
     * @param satelliteKind a String of satellite type to build "GEO", "LEO", or "DEEP"
     */
    ArrayList<Satellite> buildSatellitesByKind(Location myLocation, long time, SATDBAdapter myDBAdapter, String satelliteKind) {
        ...
        if (MainActivity.DEBUG) {
            Log.d(this.getClass().getName(), "buildSatellitesByKind() takes " + String.format("%4.2f", (System.nanoTime() - startTime) / 1000000.)
    ...
    private ArrayList<Satellite> makeSatellites(ArrayList<String[]> mTLEStrings, Location myLocation, DateTime time) {
        ...
        boolean showInactiveSatellites = defaultSettings.getBoolean(SHOW_INACTIVE_SATS_KEY, true);
     ...
     *
     * @param timeOffset is the difference between System.currentTime and the time read from Location service
     *                   (System.currentTime is not necessarily real-world time)
     ...
     */
    protected void recalculateLookAngles(ArrayList<Satellite> satList, Location myLocation, long timeOffset) {
        if (myLocation == null) {
            return;
        }
        GroundStationPosition groundStationPosition = calcGroundStation(myLocation);
        DateTime timeNow = new DateTime(new Date(System.currentTimeMillis() - timeOffset));
